...
<!-- I should keep only one of the 2 -->
use 50x stronger illumination for GFP.
compare intensity histo 

Fig \@ref(fig:high-illum-sensitivity)

comparison nb of GFP in cells with short vs long lags (center per expt, explain confounding effect: autofluo => size, day variation of bg). more sensitive than direct comparison comes from accounting for size differences?? and for local bg??
...
(ref:naive-lags-per-expt-caption) Distribution of induction lags for the *lac* operon in naive cells, stratified per day. The first panel shows all days overlaid which highlights that despite the variability between replicates, the bimodality of *lac* induction lags distributions is a robust feature.
...
(ref:lag-gfp-ini-caption) Effect of initial LacZ-GFP level on the *lac* induction lag. 
Upper panel: *lac* induction lag at the first exposure to lactose as a function of initial fluorescence. 
Lower panel: distributions of initial fluorescence in cells with short and long *lac* induction lags; the control shows the distribution of autofluorescence in a wild-type strain (fluorescence is centered per experiment, see supplementary methods xxx). We attribute the difference of fluorescence between cells with short and long lags to the LacZ-GFP molecules produced by stochastc expression before the switch.

```{r lag-gfp-ini, fig.height=4*14/9, fig.cap="(ref:lag-gfp-ini-caption)"}
plot_grid(
  plot_grid(
    myplots[['lags_gfp_scatter']] +
      theme(legend.position = 'none',
            axis.title.x = element_blank()),
...
(ref:growth-arrest-tmg-caption) xxx.

```{r growth-arrest-tmg, fig.cap="(ref:growth-arrest-tmg-caption)"}
myplots[['TMG_switch_gr_hist']]
```


(ref:lacl-gfp-gr-hists-caption) xxx.

```{r lacl-gfp-gr-hists, fig.height=5*14/9, fig.cap="(ref:lacl-gfp-gr-hists-caption)"}
plot_grid(
...
(ref:lacl-gr-lacz-caption) xxx.

```{r lacl-gr-lacz, fig.cap="(ref:lacl-gr-lacz-caption)"}
