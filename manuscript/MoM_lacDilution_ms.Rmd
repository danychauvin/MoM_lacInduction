...
A. Time series of cell length and number of LacZ-GFP molecules (on a log scale) for cells in one representative growth channel over a typical experiment (2 consecutive lactose exposures with 8 h delay); each line correspond to one cell cycle. For clarity only the 4 cells closest to the close end are shown.
...
C. Reverse cumulative distributions of *lac* induction lags at the second exposure to lactose for variable delays since the first lactose episode (in colours; 1 or 2 replicates per condition); the distribution of naive cells is shown as a reference (dark gray) along with a negative control where cells are first exposed to lactose after 26 h in the microfluidics chip (light gray).
...
A. Distribution of *lac* induction lags with increased (green) or reduced (blue) LacI activity corresponding to lower or higher expression respectively in the repressed state. Each contour corresponds to an independant replicate (with ≈150 cells). A similar result is obtained when titrating LacI before and during the exposure to lactose (Fig. \@ref(fig:basal-perturb-with)).
