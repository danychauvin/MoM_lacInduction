...
mycells_switching %>% ungroup %>% 
  ...
  mutate(lag=ifelse(!is.infinite(lag), lag, max(lag[is.finite(lag)])+60),
         lag=ifelse(is.na(lag), -1, lag)) %>% 
  (function(.df)
    ggplot(data=NULL, aes(x=lag/60, col=condition, fill=condition)) +
...
ggsave('plots/BZSposter2017/2nd_switch_lags.pdf', width=180/25.4/2, height=100/25.4/2)
...
mycells_switching %>% 
  ...
  # semi_join(myframes %>% mutate(genealogy=gsub(":", "", genealogy)), .) %>% 
  qplot(time_sec, length_um, col=interaction(date, pos, gl, genealogy), data=., geom='line') +scale_colour_periodic_brewer(guide='none')+scale_x_hours()
...
mycells_switching %>% 
  ...
  filter(is.infinite(growth_lag) | is.infinite(gfp_lag) | is.infinite(lag_200)) %>%
  # select(date, pos, gl, genealogy, t_lac_switch, t_lac_end, tmax_lag_fit)
  semi_join(myframes_switching %>% filter(fit_lag) %>% mutate(genealogy=gsub(":", "", genealogy)), .,
            by=c('date', 'pos', 'gl', 'genealogy')) %>% 
  group_by(date, pos, gl, genealogy) %>% 
  filter(time_sec==max(time_sec)) %>% # identify cells at the end of the lactose period
  select(date, pos, gl, genealogy, growth_lag, gfp_lag, lag_200) %>% 
  ...
  with(interaction(medium, m_cycle)) %>% table
  select(date, pos, gl, genealogy, id, medium, m_cycle, m_start, time_sec, end_time)
  
  # ungroup %>% filter(medium=='glucose', m_cycle==2) %>% 
  with(interaction(date, pos, gl, genealogy)) %>% unique %>% length
  filter(!discard_top) %>% 
ggplot(aes(time_sec-m_start, length_um)) +
  geom_line(aes(col=cell)) +
  scale_x_hours(2, limits=c(0, 6*3600), name='time after switch to glucose (h)') +
...
semi_join(myframes %>% mutate(genealogy=gsub(":", "", genealogy)), .) %>% 
  ...
  filter(!discard_top) %>% 
  group_by(condition, date, pos, gl, genealogy) %>% 
  summarise(n_points=n(), end_type=unique(end_type),
            #logl_time_slope=lm(log(length_um)~time_sec, data=.)$coef[2],
            logl_time_r2=cor(time_sec, log(length_um))^2)
