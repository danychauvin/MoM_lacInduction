...
filter(myframes, !discard_start, !discard_top, condition=='switch_4h', end_type=='div') %>%
  ungroup %>% filter(gl_id %in% nth(unique(gl_id), gl_idx)) %>% 
  ggplot(aes(time_sec - 2*3600, length_um, col=cell)) +
  geom_rect(aes(xmin=t_start - 2*3600, xmax=t_end - 2*3600, ymin=0, ymax=Inf, x=NaN, y=NaN, group=1), fill=rgb(1, 0, 0, .1), size=0.2, col='red', data=filter(condition_ts, condition=='switch_4h', medium=='lactose')) +
  geom_path(alpha=0.6) +
...
Plot GFP tracks of the switch_4h condition:
...
ggplot(aes(time_sec - 2*3600, gfp_nb / length_um, col=cell)) +
  geom_rect(aes(xmin=t_start - 2*3600, xmax=t_end - 2*3600, ymin=-Inf, ymax=Inf, x=NaN, y=NaN, group=1), fill=rgb(1, 0, 0, .1), size=0.2, col='red', data=filter(condition_ts, condition=='switch_4h', medium=='lactose')) +
  geom_path(alpha=0.6) +
  scale_colour_periodic_brewer(guide='none') +
  scale_x_hours(4) +
...
filter(myframes, !discard_start, !discard_top, condition=='switch_4h', end_type=='div') %>% head(1e4) %>% 
  ggplot(aes(time_sec - 2*3600, gfp_nb, col=cell)) +
  geom_rect(aes(xmin=t_start - 2*3600, xmax=t_end - 2*3600, ymin=-Inf, ymax=Inf, x=NaN, y=NaN, group=1), fill=rgb(1, 0, 0, .1), col='red', data=filter(condition_ts, condition=='switch_4h', medium=='lactose')) +
  geom_path(alpha=0.5) +
  scale_colour_periodic_brewer(guide='none') +
  scale_x_hours(5) +
  labs(y='total fluorescence (GFP molecules)') +
...
```{r fig.height=21}
myframes_switching <- filter(myframes, !discard_start, !discard_top, !(condition %in% c('glucose', 'lactose', 'lactose_lowillum')), time_sec>2*3600) %>%
...
mycells_switching <- myframes_switching %>%
  ...
  partition(condition, cell, cluster=mycluster) %>%
  do((function(.df){
...
switch_facets_labels <- function (.str) {
  .labels <- .str
  .labels[.labels=='switch_4h'] <- 'lactose (4h switch)'
  .labels[.labels=='switch_6h'] <- 'lactose (6h switch)'
  .labels[.labels=='switch_8h'] <- 'lactose (8h switch)'
  .labels[.labels=='switch_iptg'] <- 'IPTG (4h switch)'
  return(.labels)
}
...
ggplot(filter(mycells_switching, condition %in% c('switch_4h')), aes(x=lag_200/60, y=..density..)) +
...
ggplot(filter(mycells_switching, condition %in% c('switch_4h')), aes(x=lag_200/60, y=..density..)) +
  ...
  xlim(0, 250) +
  theme(legend.position=c(1, 1), legend.justification=c(1, 1)) +
  labs(x='time to induction (+200 GFP molecules; min)', y='rev. cumulative proba.', col='switch')
...
ggplot(filter(mycells_switching, str_detect(condition, 'switch_[0-9]+h'), switch_idx==1), aes(x=lag_200/60)) +
  stat_ecdf(aes(y=1-..y..),
            data=filter(mycells_switching, str_detect(condition, 'switch_[0-9]+h'), switch_idx==1)) +
  labs(x='time to induction (+200 GFP molecules; min)', y='reverse cumulative probability', title='4_6_8h')
...
```{r}
mycells_switching %>% 
  filter(condition=='switch_4h', switch_idx==2, gfp_ini<190) %>% 
  ...
  ggplot() +
  geom_rect(aes(xmin=t_start - 2*3600, xmax=t_end - 2*3600, ymin=11e3, ymax=Inf, fill=medium, group=1), size=0.2, data=filter(condition_ts, condition=='switch_4h') ) +
  geom_vline(aes(xintercept=t_start - 2*3600, group=1), alpha=.2, size=.5, data=filter(condition_ts, condition=='switch_4h')) +
...
mycells_switching %>% 
  ...
  filter(!(condition=='switch_4h' & switch_idx==2 & gfp_ini<150)) %>%
...
filter(mypairs_switching, rel!='niece') %>% 
  ...
  filter(!( (condition=='switch_4h' & switch_idx==2 & gfp_ini_1<150) |
              (condition=='switch_4h' & switch_idx==2 & gfp_ini_2<150) )) %>%
