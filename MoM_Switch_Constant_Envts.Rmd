...
```{r}
# mycells_constant %>% filter(condition %in% c('glucose', 'lactose')) %>% with(logl_time_r2>.95) %>% table
mycells_constant %>% 
  # filter(logl_time_r2>.95) %>% 
  ( function(.df)
...
mycells_constant %>% 
  filter(logl_time_r2>.98) %>%
  ggplot(aes(logl_time_slope*3600)) +
  stat_bin(aes(y=..density.., fill=condition), geom='bar', position='identity', col='transparent', alpha=.2, binwidth=2e-2) + 
  geom_step_hist(aes(y=..density.., col=condition), position='identity', binwidth=2e-2) +
  expand_limits(x=0) +
  # labs(x='doubling time (min)') +
  theme(legend.justification=c(1, 1), legend.position=c(1, 1))
...
mycells_constant %>% 
  # filter(logl_time_r2>.98) %>% 
  group_by(condition) %>% 
  # summarise(n=n(), m=mean(logl_time_r2), s=sd(logl_time_r2), cv=s/m)
  summarise(n=n(), m=mean(logl_time_slope), s=sd(logl_time_slope), cv=s/m)
...
NB: Growth rate is shown in doubling time units…
