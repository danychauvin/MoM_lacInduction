...
Let's compute instantaenous rates:

```{r eval=FALSE}
rate_width <- 10 # number of frames used for fitting
rate_hw <- round(rate_width/2) * dt * 60

myrates <- semi_join(myframes, mycells, by='cell') %>%  # keep only full cells in lactose
...
myrates <- left_join(myrates, myframes %>% select(gl_id, cell, genealogy, start_time, end_time) %>% distinct) %>% 
...
myrates %>% filter(condition %in% c('glucose', 'lactose'),
  ...
  stat_summary(aes(col=condition), fun.data=mean_cl_normal, geom="smooth")

```

xxx corresponding doubling time plot
  ...
  (function() {
    ...
    mypairs_constant %>% filter(condition_1 %in% c('glucose', 'lactose'), rel=='sisters') %>% 
      ...
      do( (function(.df){
        ...
        # browser()
        .rates <- filter(myrates_25_75, gl_id==.df$gl_id, genealogy %in% c(.df$ancestor, .df$genealogy_1, .df$genealogy_2))
      ...
      (function(.df){
        ...
        bind_rows(.df, 
                  filter(myrates_25_75, cell %in% .cells) %>% 
...
fit along 1/2 cell cycles instead of irate.
fit irate using only cell points (variable nb of points)
...
myframes %>% filter(!discard_top, !discard_start, condition=='lactose',
  ...
  ggplot(aes(time_sec, gfp_nb/length_um)) +
...
myframes %>% filter(!discard_top, !discard_start, condition=='lactose',
  ...
  group_by(lag) %>%
  summarise(acf_m=mean(acf), acf_sd=sd(acf)) %>%
  (function(.dfs) 
    ggplot(.dfs, aes(lag, log10(acf_m))) +
     geom_errorbar(aes(ymin=log10(acf_m-2*acf_sd), ymax=log10(acf_m+2*acf_sd))) +
     geom_point() +
     geom_smooth(method='lm', formula='y~x-1', data=filter(.dfs, lag<50), fullrange=TRUE) +
     # scale_y_log10() +
     xlim(0, 100) )
...
In order to compare with a random population sampling, we use a fixed number of measurements per cell (otherwise cells growing cells are overrepresented).
```{r}
