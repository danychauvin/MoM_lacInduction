...
xxx fit along 1/2 cell cycles instead of irate.
...
ggplot(aes(y=..density..)) +
  ...
  # geom_step_hist(aes(x=gfp_deriv_per_l_std, col='1/L dGFP/dt raw'), binwidth=.1, position='identity') +
  scale_y_log10() +
...
myrates %>% filter(condition %in% 'lactose') %>% 
  ...
  ungroup() %>% 
  filter(between(islope_g_l, quantile(islope_g_l, 1e-4), quantile(islope_g_l, 1-1e-4))) %>% 
  mutate(islope_g_t_per_l = islope_g_time/length_predict, gfp_deriv_per_l = gfp_deriv/length_predict) %>% 
ggplot(aes(y=..density..)) +
  geom_step_hist(aes(x=islope_g_t_per_l, col='1/L dGFP/dt'), position='identity') +
