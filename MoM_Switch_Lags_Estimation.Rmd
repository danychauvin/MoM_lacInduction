...
mycells_switching <- mycells_switching %>%
  mutate(reac_lac_end = abs(tmax_lag_fit-(t_lac_end-180))<1e-3) %>% 
  mutate(gfp_lag=ifelse(is.infinite(gfp_lag) & !reac_lac_end, NA, gfp_lag),
         growth_lag=ifelse(is.infinite(growth_lag) & !reac_lac_end, NA, growth_lag),
         lag_200=ifelse(is.infinite(lag_200) & !reac_lac_end, NA, lag_200) )
