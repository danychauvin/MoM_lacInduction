...
mycells_switching %>% ungroup() %>% 
  ...
  labs(x='time after the switch (min)', y='fraction of growth arrested cells',
       subtitle='naive cells (9 independent experiments)') +
...
mycells_switching %>% ungroup() %>% 
  ...
  (function(.df)
    ggplot(.df, aes(x=factor(1), p_arrest)) +
     ...
     # geom_col(aes(fill='growth arrest'), position='identity', 
     #          data=.df %>% filter(delay==9)) +
     # geom_col(position='identity', fill='black', alpha=.1,
     #          data=.df %>% filter(delay==15)) +

     # scale_x_discrete(breaks=c(9,15,21), labels=rep(1, 3)) +
     # labs(x='time after the switch (min)', y='fraction of growth arrested cells') +
     coord_polar(theta='y') +
labs(fill='growth arrest') +
          theme_void() +
