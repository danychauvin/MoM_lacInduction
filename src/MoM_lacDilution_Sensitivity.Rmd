...
Since length traces have more complex shapes than for the switch to lactose, it is not possible to fit a model corresponding to no growth followed by linear increase.
...
myframes_switching %>% 
  ...
  geom_freqpoly(aes(lty='reference'), binwidth=.08,
  ...
  geom_freqpoly(aes(lty='after the switch'), binwidth=.08) +
  coord_cartesian(xlim=c(-0.4, 1.2)) +
  scale_colour_discrete(labels=rename_conds) +
...
(myplots[['lacl_gfp_facets']] <-
   ...
   left_join(myframes %>% select(condition, date, pos, gl, id, time_sec, length_um, gfp_nb)) %>% 
   mutate(condition=rename_conds(condition),
          type=ifelse(islope_ll_time/log(2)*3600 > 0.1, 'growing', 'non growing')) %>% 
...
(myplots[['lacl_gr_facets']] <-
   ...
   left_join(myframes %>% select(condition, date, pos, gl, id, time_sec, length_um, gfp_nb)) %>% 
   mutate(condition=rename_conds(condition),
          type=ifelse(gfp_nb/length_um > 250, 'induced', 'non induced')) %>% 
   bind_rows(., mutate(., type='all')) %>% 
