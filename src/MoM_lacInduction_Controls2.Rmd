...
mycells_switching %>% 
  ...
  ggplot() +
  geom_rect(aes(xmin=m_start - 2*3600, xmax=m_end - 2*3600, ymin=11e3, ymax=Inf, fill=medium, group=1), size=0.2, data=condition_acq_times %>% group_by(condition, medium, m_cycle) %>% slice(1) %>% filter(condition=='switch_04h') ) +
  geom_vline(aes(xintercept=m_start - 2*3600, group=1), alpha=.2, size=.5, data=condition_acq_times %>% group_by(condition, medium, m_cycle) %>% slice(1) %>% filter(condition=='switch_04h')) +
