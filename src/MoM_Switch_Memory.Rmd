...
mycells_switching %>% ungroup %>% 
  ...
  (function(.df)
    ggplot(data=NULL, aes(x=lag/60)) +
     facet_grid(switch_idx~variable) +
     stat_ecdf(aes(y=1-..y.., col=condition), geom='line',
               data=.df %>% filter(switch_idx==2)) +
     labs(x='lag after the switch (min)', y='reverse cumulative probability') +
...
mycells_switching %>% ungroup %>% 
  ...
  mutate(lag=ifelse(lag>240*60, Inf, lag),
         # lag=ifelse(!is.infinite(lag), lag, max(lag[is.finite(lag)])+1e6),
         lag=ifelse(is.na(lag), -1e6, lag)) %>% 
  mutate(condition=str_replace(condition, 'switch_0*', '')) %>% 
  (function(.df) 
    ggplot(data=NULL, aes(x=lag/60)) +
     # stat_ecdf(aes(y=1-..y.., group=date, col='first switch'), geom='line', size=.2,
     #           data=filter(.df, switch_idx==1)) +
     stat_ecdf(aes(y=1-..y.., col='control'), geom='line', size=.6,
               data=filter(.df, switch_idx==1)) +
     stat_ecdf(aes(y=1-..y.., col=condition), geom='line', size=.3,
               data=.df %>% filter(switch_idx==2, condition!='6h')) +
     geom_rect(aes(x=NULL), xmin=-Inf, xmax=0, ymin=-Inf, ymax=Inf, fill='white', col='transparent',
               data=data.frame()) +
     scale_colour_discrete(limits=c('control', setdiff(unique(.df$condition), '6h')) ) +
     coord_cartesian(xlim=c(-10, 240), ylim=c(-.01, 1.01), expand=FALSE) +
     labs(x=expression(paste(italic('lac'), ' induction lag (min)')), 
          y='rev. cumul. probability', col='delay between\nlactose exposure') +
     theme_classic(base_size=8) + theme(axis.line.x=element_line(), axis.line.y=element_line(),
                                        strip.background=element_rect(fill='gray95', colour='transparent'), 
                                        # legend.position = c(1,1), legend.justification = c(1,1),
                                        legend.text=element_text(size=8, face='bold'), legend.key.height=unit(0.7, "lines"))
  )
# ggsave('Rplot.pdf', width=4.5, height=1.7)
...
### Memory as a function of maximum induction
...
mycells_switching_memory <-
  ...
  inner_join(
    myframes %>% ungroup %>% 
      ...
      filter(abs(time_sec-(m_end-dt*60))<1) %>%
...
mycells_switching_memory %>% 
  ...
  (function(.df) bind_rows(mutate(.df, facet=condition), mutate(.df, facet='all')) ) %>% 
  ggplot(aes(gfp_inherit, lag_200/60, col=condition, fill=condition)) + 
  facet_grid(facet~.) +
  ...
  geom_segment(aes(x=pmax(0, gfp_inherit-sqrt(gfp_inherit)), xend=gfp_inherit+sqrt(gfp_inherit),
                  yend=lag_200/60), alpha=.1) +
  geom_point(pch=21, stroke=0, alpha=.2) +
  scale_x_log10() +
  coord_cartesian(xlim=c(0.1, 1e4)) +
  expand_limits(y=0)

# ggsave('Rplot.pdf', width=6, height=10) 
...
mycells_switching_memory %>% 
  ...
  # geom_segment(x=0, xend=2, y=100, yend=100, data=NULL) +
  geom_segment(aes(x=pmax(0, gfp_inherit-sqrt(gfp_inherit)), xend=gfp_inherit+sqrt(gfp_inherit),
                  yend=lag_200/60), alpha=.05) +
  geom_point(pch=21, stroke=0, alpha=.1) +
  scale_x_log10() +
  coord_cartesian(xlim=c(0.1, 5e3), ylim=c(0, 50)) 

last_plot() +
  scale_x_continuous() +
  coord_cartesian(xlim=c(0.1, 1.5e3), ylim=c(0, 50))
...
mycells_switching_memory %>% 
  ungroup %>% 
  mutate(gfp_inherit=parent_gfp/2^divs_since_par,
         gfp_inherit_bin=as.numeric(as.character(Hmisc::cut2(gfp_inherit, cuts=c(-Inf, 10^seq(0.1, 3.8, .3)), levels.mean=TRUE))) ) %>% 
  group_by(gfp_inherit_bin) %>% 
  (function(.df) 
    ggplot(.df, aes(gfp_inherit, lag_200/60)) + 
     geom_hline(yintercept = 48, lty='dashed') +
     # geom_segment(x=0, xend=2, y=100, yend=100, data=NULL) +
     geom_segment(aes(x=pmax(0, gfp_inherit-sqrt(gfp_inherit)), xend=gfp_inherit+sqrt(gfp_inherit),
                      yend=lag_200/60, col=condition), alpha=.05) +
     geom_point(aes(fill=condition), pch=21, stroke=0, alpha=.1) +
     geom_pointrange(aes(gfp_inherit_bin, mean, ymin=mean-se, ymax=mean+se, size=n), 
                data=.df %>% group_by(gfp_inherit_bin) %>% summarise(n=n(), mean=mean(lag_200/60), se=sd(lag_200/60)/sqrt(n))) +
...
mycells_switching_memory %>% 
  ungroup %>% 
  mutate(gfp_inherit=parent_gfp/2^divs_since_par,
         gfp_inherit_bin=as.numeric(as.character(Hmisc::cut2(gfp_inherit, cuts=c(seq(-10, 1500, 100)), levels.mean=TRUE))) ) %>% 
  group_by(gfp_inherit_bin) %>% 
  (function(.df) 
    ggplot(.df, aes(gfp_inherit, lag_200/60)) + 
     geom_hline(yintercept = 48, lty='dashed') +
...
myframes %>% ungroup() %>% 
  ...
  (function(.df) 
    ggplot(.df) +
     ...
     geom_rect(aes(xmin=pmax(0, t_start - 2*3600), xmax=t_end - 2*3600, ymin=15e3, ymax=Inf, fill=medium, group=1), size=0.2, data=semi_join(condition_ts, .df, by='condition') ) +
     geom_vline(aes(xintercept=pmax(0, t_start - 2*3600), group=1), alpha=.2, size=.5, data=semi_join(condition_ts, .df, by='condition') ) +
...
mypairs_memory <- mycells_switching %>% 
  ...
  do((function(.df_gl){
    ...
    .df_gl %>% filter(switch_idx==2) %>% 
      select(genealogy_2=genealogy, lag_2=lag_200, logl_time_slope_pre_2=logl_time_slope_pre) %>% group_by(genealogy_2) %>% 
  ...
  left_join(mycells_switching %>% ungroup %>% 
              select(date, pos, gl, genealogy_1=genealogy, lag_1=lag_200, logl_time_slope_pre_1=logl_time_slope_pre) )
