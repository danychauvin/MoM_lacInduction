...
(myplots[['naive_lags_correl']] <- mycells_switching %>% ungroup %>% 
  ...
  (function(.df)
    ggplot(.df, aes(value, lag_200/60)) + 
     ...
     expand_limits(y=0) +
     guides(col=guide_legend(override.aes = list(size=2, alpha=1))) +
     labs(x='fluorescence at the switch\n(centered per experiment; equivalent GFP molecules)', y=expression(paste(italic("lac"), " induction lag (min)")), col=expression(paste("type of ", italic("lac"), " induction lag (min)"))) +
