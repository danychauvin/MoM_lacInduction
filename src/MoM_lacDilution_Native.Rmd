...
mycells_switching %>% ungroup %>% 
  ...
  (function(.df)
    ggplot(.df, aes(x=lag/60)) +
     ...
     labs(x=lac_lags_label, subtitle='naive cells') +
     coord_cartesian(xlim=c(-10, 240), expand=FALSE) +
...
mycells_switching %>% ungroup() %>% 
  ...
  expand_limits(x=0, y=0) +
  labs(subtitle='bottom cells', x='lag at first switch (min)', y='lag at second switch (min)') +
  NULL
...
mycells_switching %>% ungroup() %>% 
  ...
  geom_point(alpha=.4) +
  scale_x_continuous(breaks=seq(0, 1000, by=100)) +
  labs(subtitle='average in growth channel', x='lag at first switch (min)', y='lag at second switch (min)') +
  NULL
...
In order to estimate the inherited LacZ-GFP of a given cell at the second switch, we compute the maximum LacZ-GFP level of its ancestor (at the end of the first lactoise exposure) and account for the dilution given the number of division until the second switch (assuming even partitioning at each division).
...
mycells_switching_memory %>% ungroup() %>% 
  ...
  (function(.df) 
    ggplot() + 
     ...
     scale_x_continuous(breaks=10^(-1:3),
