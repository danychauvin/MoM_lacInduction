...
myframes_switching <- myframes %>% 
  ...
  mutate(switch_idx=first(m_cycle),
    ...
    type!='1dg' & # discard traces after one daughter is lost (because of discontinuity)
      tail(time_sec[type!='1dg']-t_lac_switch[type!='1dg'], 1) > (4 * dt*60) & # require at least 4 frames after the switch (one point after the gfp_ini window)
...
mycells_switching <- myframes_switching %>%
  ...
  partition(condition, switch_idx, date, pos, gl, ugen, cluster=mycluster %>% cluster_assign_func(compute_lag) %>% cluster_assign_obj(dt)) %>%
  do((function(.df){
    ...
    .t_200 <- filter(.df, time_sec>.tmin, gfp_nb > .preswitch_gfp+200)$time_sec %>% first # returns NA if empty
    .t_200_fail <- filter(.df, time_sec>.t_200, gfp_nb < .preswitch_gfp+200)$time_sec %>% first
    }
    
    return(data.frame(n_fit=nrow(.df), genealogy=gsub(":", "", unique(.df$genealogy)), 
...
mycells_switching %>% ungroup %>% 
  ...
  glimpse
  write_csv("share/20171212_lacLags.csv")
...
(function(.condition='switch_lacIoe') {
  .pl <- mycells_switching %>% ungroup %>% 
    filter(!date %in% discarded_dates) %>%
    filter(switch_idx==1) %>% 
    filter(str_detect(condition, '^switch_[0-9]+h$') | condition==.condition) %>% 
