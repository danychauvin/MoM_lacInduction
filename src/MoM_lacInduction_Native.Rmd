...
(myplots[['naive_lags_per_expt']] <- function(na.rm=TRUE)
  ...
  #        lag=ifelse(!na.rm & is.na(lag), -1, lag)) %>% 
  # (function(.df) bind_rows(.df, .df %>% mutate(date=NA, condition='all')) ) %>% 
    ggplot(aes(x=lag/60, y=..density..)) +
     geom_freqpoly(aes(col=factor(date)), binwidth=6, show.legend=FALSE) +
     # geom_freqpoly(aes(col=factor(date_col)), breaks=seq(0, 240, 6)-3, pad=TRUE, show.legend=FALSE,
                   # data=.df %>% mutate(date_col=date, date=NA, condition='all')) +
     labs(x=lac_lags_label) +
     coord_cartesian(xlim=c(-10, 240), expand=FALSE) +
...
myframes %>% 
  ...
  select(date, gfp_ini=gfp_nb) %>% 
  (function(.df)
    ggplot(.df, aes(gfp_ini)) + 
      geom_line(aes(gfp_ini, cdf),
                data=.df %>% 
                  ungroup %>% nest() %>% 
                  mutate(fit = map(data, ~MASS::fitdistr(.$gfp_ini, "normal") ),
                         curve = map(fit, ~tibble(gfp_ini=seq(-100, 200, 1), 
                                                  cdf=pnorm(gfp_ini, .$estimate['mean'], .$estimate['sd']))) ) %>% 
                  unnest(curve) ) +
     stat_ecdf(aes(col=factor(date))) +
     coord_cartesian(xlim=c(-50, 150)) +
     labs(x='fluo (~GFP)', y = 'cdf', col='date') +
     theme(legend.position = 'none') +
     NULL
   )
ggsave('gfp_mg1655_cdfs.pdf', width=6, height=3.5)
...
mycells_switching %>% ungroup %>% 
  ...
  scale_color_hue() +
  labs(x='fluo at switch (~GFP)', y = 'cdf', col='date')+
  NULL
ggsave('gfp_ini_cdfs.pdf', width=6, height=3.5)
...
mycells_switching %>% ungroup %>% 
  ...
  (function(.df) {
    ggplot(.df, aes(gfp_ini)) + 
      ...
      stat_ecdf(aes(col=factor(date))) +
      geom_line(aes(gfp_ini, cdf),
                data=.df %>% group_by(type) %>% 
                  filter(date != '20150703') %>% 
                  nest() %>% 
                  mutate(fit = map(data, ~MASS::fitdistr(.$gfp_ini,"normal") ),
                         curve = map(fit, ~tibble(gfp_ini=seq(-100, 200, 1), 
                                                  cdf=pnorm(gfp_ini, .$estimate['mean'], .$estimate['sd']))) ) %>% 
      ...
      coord_cartesian(xlim=c(-50, 150)) +
      scale_color_hue() +
      labs(x='fluo at switch (~GFP; centered)', y = 'cdf', col='date')+
     NULL
   })
ggsave('gfp_ini_centered_cdfs.pdf', width=6, height=3.5)
...
mypairs_switching %>%
  ...
  mutate(type_1=ifelse(value_1 < 50*60, 's', 'l'),
         type_2=ifelse(value_2 < 50*60, 's', 'l'),
         type=paste0(type_1, type_2) ) %>%
  mutate(rel=fct_relevel(rel, 'indep', 'sisters')) %>% 
  group_by(rel, type) %>% summarise(n=n()) %>%
  group_by(rel) %>% mutate(p=n/sum(n)) %>%
  summarise(n = sum(n), mi = entropy::mi.plugin(matrix(p, 2, 2)),
            nmi = mi / entropy::entropy(c(.3, .7)) ) %>% 
  ggplot(aes(rel, nmi)) +
