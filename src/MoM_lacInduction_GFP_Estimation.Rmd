...
ggplot(aes(time_sec, fluo_amplitude/length_um)) + 
  geom_rect(aes(x=1, y=1, xmin=m_start, xmax=m_end, ymin=-Inf, ymax=Inf, fill=medium), alpha=.1, data=condition_acq_times %>% group_by(condition, medium, m_cycle) %>% slice(1) %>% filter(condition=='mg1655')) + # draws one rectangle per data line
...
swi_triads <- filter(myframes, !discard_start, !discard_top, condition=='switch_04h') %>% 
  ...
  partition(date, pos, gl,
            cluster=mycluster %>% cluster_assign_obj(bleaching_rate, triad_avgw, triad_fitw) %>%
