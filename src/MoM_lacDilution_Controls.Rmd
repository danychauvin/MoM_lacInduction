...
mycells_btw %>% 
  ...
  (function(.df) {
    ...
    ggplot(.df) +
      ...
      labs(x='birth length (µm)', y='growth rate (doublings/h)', title='initial condition (between t=2h and t=6h)') +
...
mycells_switching %>% 
  ...
  ggplot() +
  geom_point(aes(logl_time_slope_before / log(2) * 3600,  lag_200/60, size=n_preswitch, col=medium_ini), alpha=0.2) +
  geom_vline(xintercept = min_growth_rate / log(2) * 3600, linetype='dotted') +
  # scale_color_gradient(low = "red", high = "blue", trans='log10') +
  # scale_size_continuous(trans='log10') +
  coord_cartesian(xlim=c(0, 1.5)) +
  labs(x='growth rate before the switch (doublings/h)', y='induction lag (min)', col='initial condition', size='points before\nthe switch') +
...
mycells_switching %>% 
  ...
  left_join(condition_ts %>% filter(t_start==0) %>% 
              select(condition, medium_ini=medium) ) %>% 
  ggplot() +
  geom_freqpoly(aes(logl_time_slope_before / log(2) * 3600, col=medium_ini), binwidth=.02) +
  geom_vline(xintercept = min_growth_rate / log(2) * 3600, linetype='dotted') +
  coord_cartesian(xlim=c(0, 1.5)) +
