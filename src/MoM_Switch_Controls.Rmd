...
mycells_btw %>% 
  ...
  (function(.df) {
    ggplot(.df, aes(log(2)/logl_time_slope / 60)) +
      ...
      coord_cartesian(xlim=c(40, 300)) +
      labs(x='doubling time (min)', title='initial growth rates (between t=2h and t=6h)')
  })
...
mycells_btw %>% 
  ...
  (function(.df) {
    ...
    ggplot(.df) +
      ...
      geom_errorbar(aes(exp(lls_med), ymin=log(2)/lltsl_pi_min / 60, ymax=log(2)/lltsl_pi_max / 60,
                        col=condition), data=.dfs ) +
      geom_errorbarh(aes(exp(lls_med), log(2)/lltsl_med / 60, xmin=exp(lls_pi_min), xmax=exp(lls_pi_max),
                        col=condition), data=.dfs ) +
...
discarded_dates <- c(20151218, # switch_08h
                    20160526, # switch_12h
                    20171122, # ∆lacA
                    20161021, 20170108, # switch_late x 2!
...
mycells_btw %>% 
  ...
  labs(x='doubling time (min)', title='growth rates after first switch (between t=1.5h and t=4h)')
