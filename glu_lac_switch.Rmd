...
scripts_path <- c("~/Documents/Biozentrum/Projects/vngWetLabR/mother_machine",
...
l_data <- mclapply(mg_files, 
                function(.f) try( load_timm_data(.f, scripts_path, .verbose=TRUE, .data2preproc=data2preproc, .force=FALSE)),
                mc.cores=numCores)
...
ggplot(data=filter(mg_frames, !discard_top, pos==0,  gl==14) %>% 
  ...
  geom_smooth(aes(height_micrometer, fluo_amplitude), method='lm', col='black', formula=y~x-1, fullrange=TRUE,
              data=filter(mg_frames, !discard_top, height_micrometer>1.7, height_micrometer<3.4))+
...
ggplot(data=myframes %>% filter(!discard_top, condition=='lactose', height_micrometer<3) %>% head(1e5),
       aes(fluo_amplitude, gfp_nb)) +
  geom_point(alpha=0.1) 

qplot(gfp_nb, data=myframes %>% filter(!discard_top, condition=='lactose', height_micrometer<3), binwidth=50)
qplot(height_micrometer, gfp_nb, data=myframes %>% filter(!discard_top, condition=='lactose', height_micrometer<3), alpha=I(.01))
```
