...
qplot(b.length_fit / (b.length_fit + t.length_fit) , b.fluo_fit / (b.fluo_fit + t.fluo_fit), data=swi_triads, geom="blank") +
  ...
  geom_point(alpha=0.5, size=1) + 
  # ylim(0.4, 0.6) +
  labs(x='length bias toward bottom daughter', y='fluorescence bias toward bottom daughter') 
...
qplot(lbias_rnd, fbias_rnd-lbias_rnd, geom="blank", 
      data=swi_triads %>% ungroup %>% 
        mutate(fbias_bottom=b.fluo_fit / (b.fluo_fit + t.fluo_fit),
               lbias_bottom=b.length_fit / (b.length_fit + t.length_fit),
  ...
  geom_point(alpha=0.2) + 
  # ylim(0.4, 0.6) +
  labs(x='volume bias', y='volume-corrected GFP bias') 
ggsave('plots/asc_nutriads_lengthcor.pdf', width=3.9, height=2.9)

with(data=swi_triads %>% mutate(fbias_bottom=b.fluo_fit / (b.fluo_fit + t.fluo_fit),
                                lbias_bottom=b.length_fit / (b.length_fit + t.length_fit)),
     corPearson(lbias_bottom, fbias_bottom-lbias_bottom) )
```
...
```{r}
mycluster %>% cluster_library("dplyr") %>% 
  cluster_library("RcppArmadillo") %>% 
  cluster_library("ccaPP")
