...
mygl <- myframes %>%
  ...
  # add curation times
  left_join(read.csv("/Users/julou/Downloads/curation_times.csv") %>%
              na.omit %>%
...
myframes <- myframes %>%
  ...
  mutate(., gfp_amplitude=(fluo_amplitude-predict(autofluo_mod, .)) * fp_per_dn)

ggplot(data=myframes %>% filter(condition=='glucose') %>% head(1e5),
       aes(fluo_amplitude, gfp_amplitude)) +
...
ggplot(filter(mycells, !discard_start, condition!='switch', end_type=='div'), # %>% filter(Rsize_vs_time^2>.98),
  ...
  # stat_bin(aes(y=..density.., col=condition), geom='stephist', position='identity') + 
    # stat_ecdf() +
