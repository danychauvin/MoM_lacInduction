...
ggplot(data=filter(mg_frames, !discard_top), aes(log(height_micrometer), log(fluo_amplitude))) + 
  geom_point(size=1, alpha=.2, position=position_jitter(width=1e-2)) + 
  geom_smooth(method='lm', col='red', fullrange=TRUE,
              data=filter(mg_frames, !discard_top, height_micrometer>1.7, height_micrometer<3.4))

ggplot(data=filter(mg_frames, !discard_top), aes(height_micrometer, fluo_amplitude)) + 
  geom_point(size=1, alpha=.2, position=position_jitter(width=1e-2)) + 
  geom_smooth(method='lm', col='red', formula=y~x-1, fullrange=TRUE,
              data=filter(mg_frames, !discard_top, fluo_amplitude>2500, height_micrometer<4))

autofluo_mod <- lm(fluo_amplitude~height_micrometer-1, data=filter(mg_frames, !discard_top, height_micrometer>1.7, height_micrometer<3.4))

```

After discarding filamentous cells (since outliers would be overweighted?!), we can use a simple linear model to predict autofluorescence for cells of a given length. Let's check visually that fluctuations for a given cell span approximately the same range (rather than having some cells systematically high or low).
...
When FPs are produced, $U_{(t)} = \alpha/\gamma + U_{(t-dt)} exp(-\gamma dt)$

At best, we also would like to estimate the total number of FPs present in the cell $P = U + B$ (instead of the unbleached fraction) in order to measure the instantaneous promoter activity. Since we know the initial condition $B_0 = 0$ and provided that we can estimate $\beta$ and $\delta$, we can compute $B_t$ for all cells at all time by traversing the tree starting from the founder cells. \
**Can we find a trick to estimate $\beta$ and $\delta$ using data from a unique illumination condition?**

$\frac{dP}{dt} = -\delta P$ \
$\frac{dU}{dt} = -(\beta + \delta) U$ \
$\frac{dB}{dt} = \beta U - \delta B$

in particular $B + \frac{dB}{dt} = \beta U + (1 - \delta) B$ hence $B_t = \beta U_{t-dt} + (1-\delta) B_{t-dt}$ (incorrect).


### Estimation

At first we focus on estimating $\beta + \delta$ and aim at extrapolating the mother and daughters fluorescence at division between the 2 adjacent time points.
...
ggplot(data = filter(myframes, !discard_start, !discard_top, condition=='switch', end_type=='div', time_sec>(2+4)*3600, 
       ...
       aes(time_sec - time_sec_ini, gfp_amplitude, col=cell)) +
  geom_path(alpha=0.6) +
  expand_limits(y=0) +
  scale_colour_periodic_brewer(guide='none') +
  scale_x_hours(0.5)  + scale_y_log10()
...
ggplot(data = filter(myframes, !discard_start, !discard_top, condition=='switch', end_type=='div', time_sec>(2+4)*3600, 
                     between_or(time_sec, 
                                ...
                                (filter(switch_ts, cond=='glucose')$t_end-30)*60) ) %>% 
         mutate(glid=paste(date, pos, gl, sep='.')) %>% filter(glid %in% nth(unique(glid), 11)) %>%
         group_by(date, pos, gl, id) %>% mutate(time_sec_ini=first(time_sec), gfp_amplitude_ini=first(gfp_amplitude)), 
       aes(time_sec - time_sec_ini, log(gfp_amplitude) -log(gfp_amplitude_ini) , group=cell, col=gfp_amplitude_ini)) +
  geom_path(alpha=0.6) +
  scale_x_hours(0.5) 
...
em_fit <- function(x,y){
  ...
  SSerr <- sum((y-yfitted)^2)
  rsquared <- 1 - (SSerr/SStot)
  
  return <- list(a=a,b=b,x=x,y=y,xfitted=xfitted,yfitted=yfitted,residuals=residuals,SStot=SStot,SSerr=SSerr,rsquared=rsquared)
}

bleaching_fits <- filter(myframes, !discard_start, !discard_top, condition=='switch', time_sec>(2+4)*3600, # end_type=='div',
                         between_or(time_sec, 
                                    (filter(switch_ts, cond=='glucose')$t_start+30)*60, 
                                    (filter(switch_ts, cond=='glucose')$t_end-30)*60) ) %>% 
  group_by(date, pos, gl, id) %>% 
  ...
  filter(npoints>10) %>%
  do(npoints=first(.$npoints), time_sec_ini=first(.$time_sec), gfp_amplitude_ini=first(.$gfp_amplitude),
     mod_lin=lm(gfp_amplitude~time_sec, data=.), mod_llog=lm(log(gfp_amplitude)~time_sec, data=.), mod_exp=em_fit(.$time_sec, .$gfp_amplitude)) %>%
  mutate(npoints=unlist(npoints), time_sec_ini=unlist(time_sec_ini), gfp_amplitude_ini=unlist(gfp_amplitude_ini),
         slope=summary(mod_lin)$coeff[2], r2=summary(mod_lin)$r.squared,
         llog_slope=summary(mod_llog)$coeff[2], llog_r2=summary(mod_llog)$r.squared,
         exp_slope=mod_exp[['b']], exp_r2=mod_exp[['rsquared']]) %>%
  mutate(cell=paste(date, pos, gl, id, sep='.'))


qplot(time_sec_ini, r2, data=bleaching_fits)

qplot(npoints, r2, data=bleaching_fits)

ggplot(data = bleaching_fits) +
  geom_point(aes(r2, exp_r2)) +
  geom_abline()
...
ggplot(data = bleaching_fits) +
  geom_histogram(aes(exp_slope)) + 
  xlim(-2e-4, 1e-4)

bleaching_rate <- mean(bleaching_fits$exp_slope, na.rm=T)
...
account for autofluo
discard bottom cell

```{r}

ggplot(data = filter(myframes, !discard_start, !discard_top, condition=='switch', time_sec>(2+4)*3600, 
                     between_or(time_sec, 
                                (filter(switch_ts, cond=='glucose')$t_start+30)*60, 
                                (filter(switch_ts, cond=='glucose')$t_end-30)*60)) %>% 
         mutate(glid=paste(date, pos, gl, sep='.')) %>% filter(glid %in% nth(unique(glid), 11)) %>%
...
triad_avgw <- 3
triad_fitw <- 10
swi_triads <- filter(myframes, !discard_start, !discard_top, condition=='switch', time_sec>(2+4)*3600, 
                     between_or(time_sec, 
                                (filter(switch_ts, cond=='glucose')$t_start+30)*60, 
                                (filter(switch_ts, cond=='glucose')$t_end-30)*60)) %>%
  mutate(fluogfp_amplitude = fluo_amplitude-predict(autofluo_mod, .)) %>%
  group_by(date, pos, gl) %>%
  do( (function(.df1){
    ...
    filter(.df1, end_type=="div") %>%
      group_by(id) %>%
       do( (function(.df1, .dfp){
         ...
         if (dim(.dfp)[1] < max(triad_avgw, triad_fitw)) return(data.frame())
         .dfcb <- filter(.df1, parent_id==unique(.dfp$id), daughter_type=='BOTTOM')
         if (dim(.dfcb)[1] < max(triad_avgw, triad_fitw)) return(data.frame())
         .dfct <- filter(.df1, parent_id==unique(.dfp$id), daughter_type=='TOP')
         if (dim(.dfct)[1] < max(triad_avgw, triad_fitw)) return(data.frame())
         # browser()
  
         fluo_modp <- em_fit(.dfp$time_sec, .dfp$fluogfp_amplitude)
         height_modp <- em_fit(.dfp$time_sec, .dfp$height_micrometer)
         fluo_modcb <- em_fit(.dfcb$time_sec, .dfcb$fluogfp_amplitude)
         height_modcb <- em_fit(.dfcb$time_sec, .dfcb$height_micrometer)
         fluo_modct <- em_fit(.dfct$time_sec, .dfct$fluogfp_amplitude)
         height_modct <- em_fit(.dfct$time_sec, .dfct$height_micrometer)
         ...
         bind_cols(
           filter(.dfp, row_number() > dim(.dfp)[1] - triad_avgw) %>%
             ...
             bind_cols(data.frame(div_time=unique(.dfp$end_time)+dt/2*60) %>% 
                         mutate(fluo_fit = exp(fluo_modp$a + div_time * fluo_modp$b),
                                height_fit = exp(height_modp$a + div_time * height_modp$b))),
           
           filter(.dfcb, row_number() <= triad_avgw) %>%
             select(id, height_avg=height_micrometer, fluo_avg=fluogfp_amplitude) %>%
             summarise_each(funs(mean)) %>%
             bind_cols(data.frame(div_time=unique(.dfp$end_time)+dt/2*60) %>% 
                         mutate(fluo_fit = exp(fluo_modcb$a + div_time * fluo_modcb$b),
                                height_fit = exp(height_modcb$a + div_time * height_modcb$b)) %>%
                         select(-div_time)) %>%
           ...
           filter(.dfct, row_number() <= triad_avgw) %>%
             ...
             summarise_each(funs(mean)) %>%
             bind_cols(data.frame(div_time=unique(.dfp$end_time)+dt/2*60) %>% 
                         mutate(fluo_fit = exp(fluo_modct$a + div_time * fluo_modct$b),
                                height_fit = exp(height_modct$a + div_time * height_modct$b)) %>%
                         select(-div_time)) %>%
...
swi_triads <- swi_triads %>%
  mutate(nu=(b.fluo_fit - t.fluo_fit)^2 / fluo_fit,
#          nu2=(b.fluo_signal - t.fluo_signal)^2 / (b.fluo_signal + t.fluo_signal),
         fluo_bin=cut2(fluo_fit, cuts=seq(0, 1e5, 5e3), oneval=FALSE),
         fluo_bin2=Hmisc::cut2(fluo_fit, g=7, oneval=FALSE))
...
#                       filter(swi_triads, between(fluo_fit, .fl-1e3, .fl+1e3)) %>% mutate(bias=b.fluo_fit / (b.fluo_fit + t.fluo_fit)) %>% .[['bias']] %>% sd)) ) +
  labs(x='mother fluorescence (DN)', y='fluorescence bias toward bottom daughter') 
...
fp_per_dn <- 0.249 # from erik's write up
```



So the autofluorescence linear model indicates that xxx conversion to equivalent gfp.

Let's check how the fluorescence measurement convert to number of gfp molecules.

```{r}

myframes <- myframes %>%
  # convert to gfp units (after subtracting autofluorescence)
  mutate(., gfp_nb = (fluo_amplitude-predict(autofluo_mod, .)) * fp_per_dn)

ggplot(data=myframes %>% filter(condition=='glucose') %>% head(1e5),
       aes(fluo_amplitude, gfp_nb)) +
  geom_point(alpha=0.1) 

ggplot(data=myframes %>% filter(condition=='lactose') %>% head(1e5),
       aes(fluo_amplitude, gfp_nb)) +
  geom_point(alpha=0.1) 
