...
mg_frames <- l_data %>% 
  ...
  mutate(b_rank=round(mean(total_cell_in_lane - cell_num_in_lane))) %>% 
  ...
  mutate(length_um=height_micrometer,
         frame=time_sec / dt / 60,
...
#   geom_rect(aes(xmin=t_start, xmax=t_end, ymin=-Inf, ymax=Inf, group=1), fill=rgb(1, 0, 0, .1), data=filter(condition_ts, condition=='switch', medium=='lactose')) +
  geom_rect(aes(xmin=dt*(frame-.5), xmax=dt*(frame+.5), ymin=-(dh*vertical_center-length_um/2), ymax=-(dh*vertical_center+length_um/2), fill=fluo_amplitude/length_um)) +
...
ggplot(data = filter(myframes, !discard_start, !discard_top, condition=='switch', end_type=='div', time_sec>(2+4)*3600, 
                     ...
                     between_or(time_sec, 
                                (filter(condition_ts, condition=='switch', medium=='glucose')$t_start+30)*60, 
                                (filter(condition_ts, condition=='switch', medium=='glucose')$t_end-30)*60) ) %>% 
  ...
  do( (function(.df1){
    ...
    filter(.df1, end_type=="div") %>%
      ...
      do( (function(.df1, .dfp){
         ...
         height_modp <- fastLmPure(cbind(1, .dfp$time_to_div), log(.dfp$length_um))
         .dfp <- mutate(.dfp, height_div = length_um / exp(height_modp$coefficients[2] * time_to_div),
                        fluogfp_div = fluogfp_amplitude / exp( bleaching_rate$mean * time_to_div))
         height_modcb <- fastLmPure(cbind(1, .dfcb$time_from_birth), log(.dfcb$length_um))
         .dfcb <- mutate(.dfcb, height_birth = length_um / exp(height_modcb$coefficients[2] * time_from_birth),
                         fluogfp_birth = fluogfp_amplitude / exp( bleaching_rate$mean * time_from_birth))
         height_modct <- fastLmPure(cbind(1, .dfct$time_from_birth), log(.dfct$length_um))
         .dfct <- mutate(.dfct, height_birth = length_um / exp(height_modct$coefficients[2] * time_from_birth),
                         fluogfp_birth = fluogfp_amplitude / exp( bleaching_rate$mean * time_from_birth))
         # browser()       
...
# #            geom_point(aes(y=log(fluogfp_amplitude), col='raw'))
         ...
         bind_cols(
           filter(.dfp, row_number() > dim(.dfp)[1] - triad_avgw) %>%
             select(end_time, height_avg=length_um, fluo_avg=fluogfp_amplitude) %>%
             summarise_each(funs(mean)) %>%
             bind_cols(data.frame(div_time=unique(.dfp$end_time)+dt/2*60, n_fit=dim(.dfp)[1]) %>%
                         mutate(fluo_fit=mean(.dfp$fluogfp_div), fluo_fit_sd=sd(.dfp$fluogfp_div)*sqrt((n_fit-1)/n_fit),
                                height_fit=mean(.dfp$height_div), height_fit_sd=sd(.dfp$height_div)*sqrt((n_fit-1)/n_fit))),
           
           filter(.dfcb, row_number() <= triad_avgw) %>%
             select(id, height_avg=length_um, fluo_avg=fluogfp_amplitude) %>%
             summarise_each(funs(mean)) %>%
             bind_cols(data.frame(n_fit=dim(.dfcb)[1]) %>%
                         mutate(fluo_fit=mean(.dfcb$fluogfp_birth), fluo_fit_sd=sd(.dfcb$fluogfp_birth)*sqrt((n_fit-1)/n_fit),
                                height_fit=mean(.dfcb$height_birth), height_fit_sd=sd(.dfcb$height_birth)*sqrt((n_fit-1)/n_fit))) %>%
             setNames(., paste("b", names(.), sep=".")),
           
           filter(.dfct, row_number() <= triad_avgw) %>%
             select(id, height_avg=length_um, fluo_avg=fluogfp_amplitude) %>%
             summarise_each(funs(mean)) %>%
             bind_cols(data.frame(n_fit=dim(.dfct)[1]) %>%
                         mutate(fluo_fit=mean(.dfct$fluogfp_birth), fluo_fit_sd=sd(.dfct$fluogfp_birth)*sqrt((n_fit-1)/n_fit),
                                height_fit=mean(.dfct$height_birth), height_fit_sd=sd(.dfct$height_birth)*sqrt((n_fit-1)/n_fit))) %>%
...
qplot(b.height_fit / (b.height_fit + t.height_fit) , b.fluo_fit / (b.fluo_fit + t.fluo_fit), data=swi_triads, geom="blank") +
  geom_abline() +
  geom_smooth(method='lm', se=FALSE) + 
  geom_point(alpha=0.2) + 
  # ylim(0.4, 0.6) +
...
```{r}
# write.csv(swi_triads %>% data.frame, file='nu_triads_20151105.csv')
...
pls <- myframes %>% 
  filter((date=='20150617' & pos==5 & gl==14))%>% 
#   filter(condition=='switch') %>% 
  ...
  do(pll=(function(.df){
    ...
    .hmin <- min(filter(.df, !discard_top)$length_um)
    ...
    list(
      l = ggplot() +
        ...
        # show cell traces
        geom_path(aes(time_sec - 2*3600, length_um, col=factor(id)), data=filter(.df, !discard_top)) +
        geom_text(aes(time_sec - 2*3600, length_um, col=factor(id), label=id), size=2, hjust=0, vjust=1,
                  data=group_by(.df, id) %>% filter(!discard_top, row_number()==1) ) +
        # show all traces in one panel
        geom_path(aes(time_sec - 2*3600, length_um, col=factor(id)), data=filter(.df, !discard_top) %>% mutate(b_rank=-1)) +
      ...
      ft = ggplot() +
        ...
        # show cell traces
        geom_path(aes(time_sec - 2*3600, gfp_nb, col=factor(id)), data=filter(.df, !discard_top)) +
        geom_text(aes(time_sec - 2*3600, gfp_nb, col=factor(id), label=id), size=2, hjust=0, vjust=1,
                  data=group_by(.df, id) %>% filter(!discard_top, row_number()==1) ) +
        # show all traces in one panel
        geom_path(aes(time_sec - 2*3600, gfp_nb, col=factor(id)), data=filter(.df, !discard_top) %>% mutate(b_rank=-1)) +
        # mask early frames (requires a dummy df!)
      ...
      fc = ggplot() +
        ...
        # show cell traces
        geom_path(aes(time_sec - 2*3600, gfp_nb/length_um, col=factor(id)), data=filter(.df, !discard_top)) +
        geom_text(aes(time_sec - 2*3600, gfp_nb/length_um, col=factor(id), label=id), size=2, hjust=0, vjust=1,
                  data=group_by(.df, id) %>% filter(!discard_top, row_number()==1) ) +
        # show all traces in one panel
        geom_path(aes(time_sec - 2*3600, gfp_nb/length_um, col=factor(id)), data=filter(.df, !discard_top) %>% mutate(b_rank=-1)) +
        # mask early frames (requires a dummy df!)
        geom_rect(aes(xmin=-Inf, xmax=0, ymin=-Inf, ymax=Inf, group=1), fill='white', alpha=.6, data=data.frame(a=1)) +
        scale_colour_periodic_brewer(guide='none') + 
        scale_fill_manual(values=c('glucose'=.fill[1], 'lactose'=.fill[2]), guide='none') +
...
mycells <- mutate(mycells, height_um_end = h_end$length_um)
