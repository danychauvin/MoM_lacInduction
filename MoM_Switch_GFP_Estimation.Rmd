...
p_decayGFP_lin <- myframes %>% 
  filter(!discard_start, !discard_top, condition=='switch_04h', end_type=='div') %>% 
  ...
  filter(n() > 10) %>% # discard cells with less than 3 points
...
filter(myframes, !discard_start, !discard_top, condition=='switch_04h') %>% 
  ...
  ungroup %>% filter(gl_id %in% nth(unique(gl_id), 10)) %>%
  group_by(date, pos, gl, id) %>% 
  mutate(npoints=sum(fluogfp_amplitude>0)) %>%
  filter(npoints>10) %>%
  mutate(time_sec_ini=first(time_sec)) %>% 
...
The variable `gfp_nb` must be corrected to account for the lower illumination.
